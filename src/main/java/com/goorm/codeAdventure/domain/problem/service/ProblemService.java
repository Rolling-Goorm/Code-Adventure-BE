...
public class ProblemService {
     ...
     * 자바, 자바스크립트 컴파일러
     */
    public void solveCode(Long problemId, String language, String sourceCode, User user) throws Exception {
        CompileResult compileResult = compileCode(problemId, language, sourceCode);

        // 컴파일한 결과를 가지고 시도횟수 차감 혹은 코인지급
        if (compileResult.isSuccess()) {
            attemptRepository.save(new Attempt(AttemptResult.SUCCESS, sourceCode, compileResult.getSubmitTime(), user, problemRepository.findById(problemId).get()));
            ...
            attemptRepository.save(new Attempt(AttemptResult.FAIL, sourceCode, compileResult.getSubmitTime(), user, problemRepository.findById(problemId).get()));
